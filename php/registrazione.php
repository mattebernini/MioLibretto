    ...
    if (isset($_POST['register'])) {
        ...
        if (empty($username) || empty($password)) {
            ...
            if ($stmt = $con->prepare('SELECT username FROM utente WHERE username = ?'))
            {
                ...
                {
                    $sql = 'INSERT INTO utente (username, password, email, facolta, anni)
                    VALUES ("'.$username.'", "'.$password_hash.'", "'.$email_ck.'", "'.$facolta.'", "'.$anni.'");';
        ...
        printf($msg, '<a href="iscriviti.html">torna indietro</a>');
    }
