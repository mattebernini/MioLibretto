    ...
    if ($stmt = $con->prepare('SELECT password FROM utente WHERE username = ?')) {
        ...
        if ($stmt->num_rows > 0)    // esiste l'username?
        {
            ...
            if (password_verify($_POST['password'], $password)) {  // verifica password
				...
				$query = "SELECT anni FROM utente WHERE username = '{$_POST['username']}'";
				$result = $con->query($query);
				while($row = $result->fetch_array())
					$_SESSION['anni_laurea'] = $row['anni'];
				header('Location: libretto.php');    // apre la pagina del gioco da loggati
        	} else {
        		// Incorrect password
        		echo 'Password e/o Username errati...'.'<a href="index.html">RIPROVACI</a>';
        	}
        } else {
        	// Incorrect username
        	echo 'Password e/o Username errati...'.'<a href="index.html">RIPROVACI</a>';
